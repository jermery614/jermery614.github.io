...
function getLarge() {
    ...
    .then(res => {
        speciesEL.innerHTML =`-> <u>Name</u>: ${res.fish[0].name}`+"<br>"+`-> <u>Location</u>: ${res.fish[0].location}`+"<br>"+`-> <u>Tips</u>: ${res.fish[0].tips}`+"<br>"+`-> <u>Tackle</u>: ${res.fish[0].tackle}`+"<br>"+`-> <u>Pre-Summer</u>: ${res.fish[0].presummer}`+"<br>"+`-> <u>Summer</u>: ${res.fish[0].summer}`+"<br>"+`-> <u>Fall</u>: ${res.fish[0].fall}`+"<br>"+`-> <u>Winter</u>: ${res.fish[0].winter}`+"<br>"+`-> <u>Spring</u>: ${res.fish[0].spring}`
    })
...
function getPike(){
    ...
    // .then(res => console.log(res))
    .then(res => {
        speciesEL.innerHTML =`-> <u>Name</u>: ${res.fish[2].name}`+"<br>"+`-> <u>Location</u>: ${res.fish[2].location}`+"<br>"+`-> <u>Tips</u>: ${res.fish[2].tips}`+"<br>"+`-> <u>Tackle</u>: ${res.fish[2].tackle}`+"<br>"+`-> <u>Pre-Summer</u>: ${res.fish[2].presummer}`+"<br>"+`-> <u>Summer</u>: ${res.fish[2].summer}`+"<br>"+`-> <u>Fall</u>: ${res.fish[2].fall}`+"<br>"+`-> <u>Winter</u>: ${res.fish[2].winter}`+"<br>"+`-> <u>Spring</u>: ${res.fish[2].spring}`
    })
    .catch(err => console.error(err));
